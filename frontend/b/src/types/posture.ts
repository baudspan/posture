...
export interface RawMetrics {
  ...
  // MOCKED until Pose — same keys the real backend will use
...
export interface PostureScore {
  status: PostureStatus;      // Derived or from backend posture_score
  violationCount: number;     // How many thresholds are currently violated
}
...
// Extended fields sent by the real Python backend (server.py)
declare module "./posture" {}
export interface RawMetricsExtended extends RawMetrics {
  head_roll_deg:  number;   // left/right tilt in degrees
  posture_ok:     boolean;
  pitch_ok:       boolean;
  roll_ok:        boolean;
  frame_b64:      string;   // base64-encoded JPEG from OpenCV
}

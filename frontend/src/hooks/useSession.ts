...
import { saveSessionFS } from "../lib/firestore";
...
export const useSession = (metrics: RawMetrics | null, currentStatus: PostureStatus) => {
  ...
  const endSession = (userId: string): SessionHistoryItem | null => {
    ...
    // Persist to localStorage (offline fallback) AND Firestore (source of truth)
    saveSession(sessionItem);
    saveSessionFS(userId, sessionItem).catch(err =>
      console.error("[Firestore] saveSession failed:", err)
    );
    resetRefs();
    return sessionItem;
  };
